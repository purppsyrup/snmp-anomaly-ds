...
# Corrected OID to name dictionary
OID_TO_NAME = {
...
# Metric names
METRIC_NAMES = list(OID_TO_NAME.values())
        ...
        with open('debug.log', 'r') as file:
            for line in file:
                if 'Retrieved SNMPv2-SMI' in line:
                    ...
                    values_part = parts[2].split(': ')[1].strip('[]\n').split()
                    values = list(map(int, values_part))
                    ...
                    line_continued = lines[i].strip()  # Read the next line for the values
                    line += ' ' + line_continued
                    parts = line.split(' - ')
                    timestamp = pd.Timestamp(parts[0])
                    values_part = parts[2].split(': ')[1].strip('[]\n').split()
...
with col2:
    st.subheader('Current Values Table')
    ...
    st.subheader('Current Values Graphs by MIB Group')
    mib_groups = {
    ...
    for tab_title in tab_titles:
        with tabs[tab_titles.index(tab_title)]:
            for metric in mib_groups[tab_title]:
                if metric in metrics_diff_df.columns:
                    ...
                    st.warning(f"No data for metric {metric}")
