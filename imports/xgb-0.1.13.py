...
xgb_model = load('xgb-0.1.13.joblib')
scaler = load('scaler-0.1.13.joblib')
label_encoder = load('enc-0.1.13.joblib')
scaler_params = np.load('params-0.1.13.npy', allow_pickle=True).item()

previous_metrics = None

def get_snmp_metrics(host, community_string):
    oids = [
        '1.3.6.1.2.1.2.2.1.10.3',    # ifInOctets3
        '1.3.6.1.2.1.2.2.1.13.3',    # ifInDiscards3
        '1.3.6.1.2.1.2.2.1.16.3',    # ifOutOctets3
        ...
        '1.3.6.1.2.1.4.8.0',          # ipInAddrErrors
        '1.3.6.1.2.1.4.11.0',         # ipOutDiscards
        '1.3.6.1.2.1.5.1.0',          # icmpInMsgs
        '1.3.6.1.2.1.5.2.0',          # icmpOutMsgs
        '1.3.6.1.2.1.5.3.0',          # icmpInDestUnreachs
        '1.3.6.1.2.1.5.4.0',          # icmpOutDestUnreachs
        '1.3.6.1.2.1.5.8.0',          # icmpInEchos
        '1.3.6.1.2.1.5.9.0'           # icmpOutEchoReps
    ]
...
def append_to_csv(metrics_diff, prediction, csv_file):
    column_names = [
        'ifInOctets11_diff', 'ifInDiscards11_diff', 'ifOutOctets11_diff',
        'tcpInSegs_diff', 'tcpRetransSegs_diff', 'tcpEstabResets_diff', 'tcpOutSegs_diff',
        'udpInDatagrams_diff', 'udpOutDatagrams_diff', 'udpNoPorts_diff',
        'ipInReceives_diff', 'ipInDelivers_diff', 'ipOutRequests_diff', 'ipInDiscards_diff', 
        'ipInAddrErrors_diff', 'ipOutDiscards_diff',
        'icmpInMsgs_diff', 'icmpOutMsgs_diff', 'icmpInDestUnreachs_diff', 'icmpOutDestUnreachs_diff', 
        'icmpInEchos_diff', 'icmpOutEchoReps_diff', 'class'
    ]
...
def main():
    ...
    csv_file = 'xgb-0.1.13.csv'
    diff_csv_file = 'diff-0.1.13.csv'
    ...
    while True:
        ...
        if prediction is not None:
            ...
            append_to_csv(metrics_diff, prediction, diff_csv_file)
