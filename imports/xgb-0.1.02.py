...
xgb_model = load('xgb-0.1.02.joblib')
scaler = load('scaler-0.1.02.joblib')
label_encoder = load('enc-0.1.02.joblib')

scaler_params = np.load('params-0.1.02.npy', allow_pickle=True).item()
...
def append_to_csv(metrics, prediction, csv_file):
    column_names = [
        ...
        "tcpRetransSegs", "tcpActiveOpens", "udpInDatagrams", 
...
def append_diff_to_csv(metrics_diff, prediction, csv_file):
    column_names = [
        ...
        "tcpOutSegs_diff", "tcpPassiveOpens_diff", "tcpRetransSegs_diff", 
...
def main():
    ...
    csv_file = 'xgb-0.1.02.csv'
    diff_csv_file = 'diff-0.1.02.csv'
